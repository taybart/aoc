...
pub fn solve(input: String) -> Result<String, String> {
    ...
    for m in moves {
        for _ in 0..m.amount {
            ...
            if let Some(from) = stacks.get_mut(&m.from) {
                update = from.pop_back().unwrap();
            }
            if !update.is_empty() {
                if let Some(to) = stacks.get_mut(&m.to) {
                    to.push_back(update.clone());
                }
            }
        }
    }
    ...
    for i in 1..stacks.len() + 1 {
        solution.push_str(stacks.get(&(i as u32)).unwrap().back().unwrap());
    }
...
mod tests {
    ...
    fn part1_example() -> Result<(), String> {
        ...
        assert_eq!(solve(example_input)?, "CMZ");
        Ok(())
    }
    #[test]
    fn part1() -> Result<(), String> {
        ...
        assert_eq!(solve(input)?, "TBVFVDZPN");
        Ok(())
    }
}
